...
class MissionManagerNode(Node):
    def __init__(self):
        ...
        # Subscriptions (RSCP Köprü Düğümünden Gelen Atomik Komutlar)
        ...
        self.create_subscription(NavSatFix, '/rscp/command/search_area', self.on_search_area_msg, 10)
        self.create_subscription(NavSatFix, '/rscp/command/navigate_gps', self.on_navigate_gps_msg, 10)
        ...
        # Motor ve Mod Komut Yayıncıları (STM32 H7)
        self.pub_h7_cmd = self.create_publisher(String, '/earendil/control/command', 10)
        self.pub_mode_req = self.create_publisher(String, '/earendil/control/mode_request', 10)
    ...
    def on_arm_disarm(self, msg: Bool):
        ...
        if self.is_armed:
            ...
            self.get_logger().info("🟢 Araç Otonom Çalışma İçin ARMED Edildi. H7 Mode: autonomous")
        else:
            mode_msg.data = "disarm"
            self.pub_mode_req.publish(mode_msg)
            self.stop_robot()
            self.get_logger().warn("🛑 Araç DISARM Edildi. H7 Mode: disarm, Motorlar Durduruluyor.")

    def on_search_area_msg(self, msg: NavSatFix):
        self.search_center_lat = msg.latitude
        self.search_center_lon = msg.longitude
        self.search_radius = msg.altitude if msg.altitude > 0 else self.search_radius
        self.get_logger().info(
            f"📍 Atomik SearchArea Alındı: Merkez=({self.search_center_lat:.6f}, {self.search_center_lon:.6f}), Yarıçap={self.search_radius:.1f}m"
        )
    ...
    def trigger_search_mission(self):
        ...
        if self.current_stage == 1:
            self.active_search_lat = self.search_center_lat
            self.active_search_lon = self.search_center_lon
    ...
    def on_navigate_gps_msg(self, msg: NavSatFix):
        self.nav_target_lat = msg.latitude
        self.nav_target_lon = msg.longitude
        self.get_logger().info(
            f"📍 Atomik NavigateToGPS Alındı: Hedef=({self.nav_target_lat:.6f}, {self.nav_target_lon:.6f})"
        )
    ...
    def on_navigator_arrived(self, msg: Bool):
        ...
        if self.current_stage == 1:
            ...
            goal_msg.header.stamp = self.get_clock().now().to_msg()
            goal_msg.header.frame_id = 'gps'
            goal_msg.latitude = getattr(self, 'active_search_lat', 0.0)
            goal_msg.longitude = getattr(self, 'active_search_lon', 0.0)
            self.pub_peak_finder_start.publish(goal_msg)

        elif self.current_stage == 2:
            self.get_logger().info("🏁 Stage 2 arama merkezine ulaşıldı. (Kamera algılaması bekleniyor).")

        elif self.current_stage == 3:
            # Lava tube girişine varıldı ➔ Hakeme TaskFinished gönder (non-blocking)
            self.get_logger().info("🏁 Lava tube girişine ulaşıldı. Hakeme TaskFinished iletiliyor...")
            self.create_timer(self.task_finished_delay_s, self._delayed_stage3_finished)
    ...
    def _delayed_stage3_finished(self):
    ...
    def on_peak_found(self, msg: NavSatFix):
        ...
        if self.current_stage == 1:
            ...
            self.create_timer(self.task_finished_delay_s, self._delayed_stage1_finished)

    def _delayed_stage1_finished(self):
        self.pub_task_finished.publish(Empty())
    ...
    def on_rock_point(self, msg: Point):
        ...
        if self.current_stage == 2 and self.is_armed and not self.rock_found_sent:
            if self.rock_visible and self.rock_distance <= self.basalt_rock_arrival_distance:
                ...
                self.create_timer(self.task_finished_delay_s, self._delayed_stage2_finished)

    def _delayed_stage2_finished(self):
        self.pub_task_finished.publish(Empty())
