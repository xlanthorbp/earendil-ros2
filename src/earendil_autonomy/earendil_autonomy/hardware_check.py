...
class HardwareCheckerNode(Node):
    def __init__(self):
        ...
        self.last_msg_times = {
            ...
            'H7 Serial Link (/earendil/h7/connected)': 0.0,
            'Wheel Telemetry (/earendil/wheels/states)': 0.0,
            'GPS Fix (/gps/fix)': 0.0
        }
        ...
        self.create_subscription(Bool, '/earendil/h7/connected', self.h7_cb, 10)
        try:
            from earendil_interfaces.msg import WheelTelemetryArray
            self.create_subscription(WheelTelemetryArray, '/earendil/wheels/states', self.wheel_cb, 10)
        except ImportError:
            pass
    ...
    def h7_cb(self, msg: Bool):
        if msg.data:
            self.last_msg_times['H7 Serial Link (/earendil/h7/connected)'] = self.get_clock().now().nanoseconds / 1e9

    def wheel_cb(self, msg):
        self.last_msg_times['Wheel Telemetry (/earendil/wheels/states)'] = self.get_clock().now().nanoseconds / 1e9
