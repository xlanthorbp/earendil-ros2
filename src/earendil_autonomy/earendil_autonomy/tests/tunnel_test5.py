    ...
    def scan_callback(self, msg):
        ...
        if self.state == 'SEARCHING_ENTRANCE':
            ...
            if abs(error) < math.radians(2.0):
                ...
                v = 0.0
                w = angular_speed
                self.get_logger().info(f"Turning parallel: error={math.degrees(error):.1f}deg, cmd_z={w:.2f}", throttle_duration_sec=0.5)
            ...
            if abs(error) < math.radians(2.0):
                ...
                v = 0.0
                w = angular_speed
                self.get_logger().info(f"Turning to face wall: error={math.degrees(error):.1f}deg, cmd_z={w:.2f}", throttle_duration_sec=0.5)
            ...
            if left_dist < self.tunnel_detection_threshold and right_dist < self.tunnel_detection_threshold:
                ...
                if self.aruco_visible:
                    w = -self.kp_aruco * self.aruco_angle + self.kp_width * self.aruco_width_diff
                    self.get_logger().info(f'Approaching Entrance - Correcting: angle={self.aruco_angle:.1f}deg, width_diff={self.aruco_width_diff:.3f}, cmd_z={w:.2f}', throttle_duration_sec=1.0)
                else:
                    w = 0.0
            ...
            if self.aruco_visible:
                v = self.forward_speed_approach
                w = -self.kp_aruco * self.aruco_angle + self.kp_width * self.aruco_width_diff
                self.get_logger().info(f'Approaching Exit - Correcting: angle={self.aruco_angle:.1f}deg, width_diff={self.aruco_width_diff:.3f}, cmd_z={w:.2f}', throttle_duration_sec=1.0)
            else:
