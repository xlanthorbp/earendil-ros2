...
class BaseEnterNode(Node):
    def __init__(self):
        ...
        self.task_finished_pub = self.create_publisher(Empty, '/rscp/feedback/task_finished', 10)
    ...
    def _load_base_position(self):
        ...
        if not os.path.exists(save_path):
            self.get_logger().warn(f"⚠️ Base position file not found at {save_path}. Defaulting base target to (0.0, 0.0).")
            self.target_x = 0.0
            self.target_y = 0.0
            self.target_yaw = 0.0
            return
        ...
        try:
            ...
            self.get_logger().error(f"❌ Failed to load base position from file: {e}. Defaulting to (0.0, 0.0).")
            self.target_x = 0.0
            self.target_y = 0.0
            self.target_yaw = 0.0

    def heading_callback(self, msg: Float64):
        # Heading from H7 in degrees -> convert to radians [-pi, pi]
        deg = float(msg.data)
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if condition_met:
            ...
            self.task_finished_pub.publish(Empty())
            self.get_logger().info("🏁 Base entrance task finished! TaskFinished feedback published.")
