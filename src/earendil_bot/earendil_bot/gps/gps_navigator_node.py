...
class GPSNavigatorNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Float32, '/mag/heading', self.heading_cb, 10)

        # RSCP ve Sistem Komutları
        ...
        self.create_subscription(Float64, '/rscp/command/navigate_gps_lat', self.nav_lat_cb, 10)
        self.create_subscription(Float64, '/rscp/command/navigate_gps_lon', self.nav_lon_cb, 10)
        self.create_subscription(Float64, '/rscp/command/search_area_lat', self.search_lat_cb, 10)
        self.create_subscription(Float64, '/rscp/command/search_area_lon', self.search_lon_cb, 10)
        
        # Doğrudan Goal ve İptal Topic'leri
    ...
    def nav_lat_cb(self, msg: Float64):
        self.target_lat = msg.data
        self.check_and_start_goal()

    def nav_lon_cb(self, msg: Float64):
        self.target_lon = msg.data
        self.check_and_start_goal()

    def search_lat_cb(self, msg: Float64):
        self.target_lat = msg.data
        self.check_and_start_goal()

    def search_lon_cb(self, msg: Float64):
        self.target_lon = msg.data
        self.check_and_start_goal()
    ...
    # ---------------------------------------------------------
    # Motor Komut Üretici (H7 Command + Twist)
    # ---------------------------------------------------------
